...
export function effectPass<U extends EffectUniforms>(options: EffectPassOptions<U>): EffectPass<U> {
  const { target = floatTargetConfig, resolutionScale = 1 } = options;

  const renderPass = quadRenderPass(undefined, { ...options, target: null });
